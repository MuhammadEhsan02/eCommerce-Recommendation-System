...
# Define these variables and functions before using them
def calculate_embeddings(img, input_shape, model):
    img_arr = cv2.imread(img)

    # Check if the image is loaded successfully
    if img_arr is None:
        raise ValueError(f"Unable to load image: {img}")

    # Ensure the image has the correct shape
    if img_arr.shape[:2] != input_shape:
        img_arr = cv2.resize(img_arr, input_shape)
...
def filter_product_ids(df, query_id):
    reference_categories = df.loc[df['id'] == query_id, ["articleType", "gender"]]
...
st.text("Search for your favorite fashion product image")
...
if uploaded_file is not None:
    ...
    # Extract the query_id from the uploaded image's name
    ...
    st.write("Please upload an image to see recommendations.")
    st.stop()

# Continue with recommendations if an image is uploaded
st.write("Just a second...")
st.write("See our recommendations")
...
st.image(query_img_path, caption='Uploaded Image.', width=400)

# Display the recommended images
recommended_images = [Image.open(image_path) for image_path in recommended_images]
st.write("Recommended Products:")
st.image(recommended_images, width=400, caption=recommended_product_confidence)
